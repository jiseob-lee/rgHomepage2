var appCommentList = angular.module('rg.CommentList', ['angularMoment', 'ngMaterial', 'ngMessages']);

appCommentList.controller('CommentListCtrl', ['$scope', '$http', '$routeParams', '$log', '$location', 
	...
	function ($scope, $http, $routeParams, $log, $location) {
	    ...
	    if ($location.path().indexOf("/getCommentTotalList") == 0) {
			// IP 접속현황을 구한다.
			$http.get("/rg/getCommentTotalList.do?pageNo=" + $scope.pageNo).then(function(response) {
