    ...
    function ($scope, $log, $http, $compile, $mdDialog, $q, $location, $routeParams) {
		...
		*/
		...
		$scope.checkTyping = typing.checkTyping = function(index, val, trim) {
			...
			$("[id^=sourceSpan_]").css("color", "white");
			$("[id^=sourceSpan_" + index + "_]").css("color", "#e8e8e8");
			...
			if (target == "") {
				...
				$("[id^=sourceSpan_" + index + "_]").css("background-color", "#053c4f");
		...
		typing.getContent = function(bibleName, bibleChapters, abbreviationKor, abbreviationEng) {
			...
			$http(req).then(function successCallback(response) {
				...
				for (var i = 0; i < typing.count; i++) {
					...
					var input = angular.element('<input class="typing2" id="typing_' + i + '" ng-model="typing[' + i + ']" type="text" my-enter="checkResult(' + i + ')" onfocus="this.oldvalue = this.value;" onkeydown="this.oldvalue = this.value;" ng-keyup="checkKeyUp($event, ' + i + ')" ng-blur="checkTypingBlur(' + i + ')" ng-click="checkTyping(' + i + ')" ng-trim="false" onpaste="return false;" ondrop="return false;" autocomplete="off" />');
