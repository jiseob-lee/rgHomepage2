var appLoginLog = angular.module('rg.LoginLog', ['angularMoment', 'ngMaterial', 'ngMessages']);

appLoginLog.controller('LoginLogCtrl', ['$scope', '$http', '$routeParams', '$log', '$location', 
	...
	function ($scope, $http, $routeParams, $log, $location) {
	    ...
	    if ($location.path().indexOf("/getLoginLog") == 0) {
			// IP 접속현황을 구한다.
			$http.get("/rg/getLoginLog.do?pageNo=" + $scope.pageNo).then(function(response) {
	
				//$log.debug("getIpCount.do", response.data);
				
				$scope.totalCount = response.data.totalCount;
				$scope.loginLogList = response.data.list;
