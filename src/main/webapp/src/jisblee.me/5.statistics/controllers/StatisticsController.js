var appStatistics = angular.module('rg.Statistics', ['angularMoment', 'ngMaterial', 'ngMessages']);

appStatistics.controller('StatisticsCtrl', ['$scope', '$http', '$routeParams', '$log', '$location', 

	function ($scope, $http, $routeParams, $log, $location) {

		var statistics = this;
		
		$scope.ipCountList = null;
	
	    $scope.pageNo = typeof $routeParams['pageNo'] == "undefined" || $routeParams['pageNo'] == "" ? "1" : $routeParams['pageNo'];
	
	    //$log.debug("$location", $location);
	    
	    if ($location.path().indexOf("/getIpCount") == 0) {
			// IP 접속현황을 구한다.
			$http.get("/rg/getIpCount.do?pageNo=" + $scope.pageNo).then(function(response) {
	
				//$log.debug("getIpCount.do", response.data);
				
				$scope.totalCount = response.data.totalCount;
				$scope.ipCountList = response.data.list;
